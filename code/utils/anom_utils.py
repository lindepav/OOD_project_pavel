...
def get_measures(out_scores, in_scores, recall_level=recall_level_default):
    pos = np.array(out_scores[:]).reshape((-1, 1))
    neg = np.array(in_scores[:]).reshape((-1, 1))
    preds = np.squeeze(np.vstack((pos, neg)))
    labels = np.zeros(len(preds), dtype=np.int32)
    labels[:len(pos)] += 1

    auroc = sk.roc_auc_score(labels, preds)
    aupr = sk.average_precision_score(labels, preds)
    fpr, threshold = fpr_and_fdr_at_recall(labels, preds, recall_level)
