...
def get_gradnorm_score(loader, model, clsfier, num_classes, args, k=20, temperature=1, name=None):
    print(args.save_path + name + ".npy", os.path.exists(args.save_path + name + ".npy"))
    if not (os.path.exists(args.save_path + name + ".npy")):
        scores = []
        ...
        for b, (x, y) in enumerate(loader):
            if b % 100 == 0:
                print(f'[{b}/{len(loader)}] batches processed')
            images = Variable(x.to(device), requires_grad=True)
            ...
            nnOutputs = nnOutputs / temperature
            loss = torch.mean(torch.sum(-targets * logsoftmax(nnOutputs), dim=-1))

            loss.backward()

            layer_grad = clsfier.cls.weight.grad.data
            layer_grad_norm = torch.sum(torch.abs(layer_grad)).cpu().numpy()
            scores.append(layer_grad_norm)

        return np.array(scores)
    else:
        scores = np.load(args.save_path + name + ".npy")
