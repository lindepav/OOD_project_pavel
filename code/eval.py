...
def evaluation():
    ...
    # in_dis
    if args.ood == "GradNorm":
        args.batch_size = 1
        args.save_path = './gradnorm' + args.save_path[8:]
    ...
    use_cpu = True
    device = get_device(cpu_only=use_cpu)
    ...
    if args.ood_data == "imagenet":
        if args.dataset == "nus-wide":
            ood_root = "./ImageNet22/nobackup-slow/dataset/nus-ood/"
            out_test_data = torchvision.datasets.ImageFolder(ood_root, transform=img_transform)
        else:
            ood_root = "./ImageNet22/nobackup-slow/dataset/ImageNet22k/ImageNet-22K"
            out_test_data = torchvision.datasets.ImageFolder(ood_root, transform=img_transform)
    elif args.ood_data == "texture":
        ood_root = "./Textures/nobackup-slow/dataset/dtd/images/"
        out_test_data = torchvision.datasets.ImageFolder(ood_root, transform = img_transform)
    elif args.ood_data == "taco":
        ood_root = "./TACO/data/"
        out_test_data = torchvision.datasets.ImageFolder(ood_root, transform=img_transform)
    elif args.ood_data == "species":
        ood_root = "./Species/images/"
        out_test_data = torchvision.datasets.ImageFolder(ood_root, transform=img_transform)
        ...
        out_test_data = torchvision.datasets.MNIST('./ImageNet22/nobackup-slow/dataset/MNIST/',
    ...
    #     clsfier = nn.DataParallel(clsfier, device)
        # model = nn.DataParallel(model, device)
        # clsfier = nn.DataParallel(clsfier, device)
    # model = model.to(device)
    # clsfier = clsfier.to(device)

    model.load_state_dict(torch.load(args.load_model + args.arch + '.pth', map_location=device), strict=True)
    clsfier.load_state_dict(torch.load(args.load_model + args.arch + 'clsfier.pth', map_location=device), strict=True)
    model.to(device)
    clsfier.to(device)
    print("model loaded!")
    ...
    if args.ood == "odin":
        ...
        args.batch_size = 1
        args.save_path = './gradnorm' + args.save_path[8:]
        in_scores = lib.get_gradnorm_score(in_test_loader, model, clsfier, args.n_classes, args, name="in_test")
        out_scores = lib.get_gradnorm_score(out_test_loader, model, clsfier, args.n_classes, args, name="out_test")
    else:
